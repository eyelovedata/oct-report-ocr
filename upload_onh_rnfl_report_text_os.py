...
fieldnames =['onh_rnfl_report_number',
            ...
            'Rim_Area_OS',
            'Disc_Area_OS',
            'Average_C_D_Ratio_OS',
            'Vertical_C_D_Ratio_OS',
            'Cup_Volume_OS',
            'S_OS',
            'T_OS',
            'N_OS',
...
query="""SELECT *
FROM `{project_id}.{dataset_id}.{table_id}`where DocumentTitle like '%Cirrus_OS_ONH and RNFL OU Analysis%'
...
if not os.path.exists('onh_rnfl_text_extraction_os.csv'):
    with open('onh_rnfl_text_extraction_os.csv', 'w', newline ='') as csvfile:
        ...
        for index in tqdm(range(len(df))):
            ...
            try:
                ...
                with open(f'onh_rnfl/onh_rnfl_os_{index+1}.pdf', 'wb') as fp:
                    fp.write(ds.EncapsulatedDocument)
                images = convert_from_path(f'onh_rnfl/onh_rnfl_os_{index+1}.pdf')

                for i in range(len(images)):
                    # Save pages as images in the pdf
                    images[i].save(f'onh_rnfl/onh_rnfl_os_{index+1}.jpg', 'JPEG')

                im = Image.open(f'onh_rnfl/onh_rnfl_os_{index+1}.jpg')
                ...
                im.save(f"onh_rnfl/onh_rnfl_os_{index+1}.jpg")

                ocr = PaddleOCR(use_angle_cls=True, lang='en') # need to run only once to download and load model into memory
                img_path = f'onh_rnfl/onh_rnfl_os_{index+1}.jpg'
                ...
                os.system('rm -rf onh_rnfl/*')
                 ...
                 print('-----------')
upload_blob('stanfordoptimagroup', 'onh_rnfl_text_extraction_os.csv', 'onh_rnfl_text_extraction_os.csv',verbose=False)
onh_rnfl_text_extraction = pd.read_csv('onh_rnfl_text_extraction_os.csv')
onh_rnfl_text_extraction = pyarrow_dtype_format_correction(onh_rnfl_text_extraction)
saved_table_id = 'imaging'+f".onh_rnfl_text_extraction_os"
