...
where DocumentTitle like "%OS Ganglion Cell OU Analysis%"
"""
query_job =client.query(query)
df=query_job.to_dataframe()
df.columns = map(str.lower, df.columns)

if not os.path.exists('gcc_text_extraction_os.csv'):
    with open('gcc_text_extraction_os.csv', 'w', newline ='') as csvfile:
        ...
        for index in tqdm(range(len(df))):
            ...
            try:
                ...
                with open(f'gcc/gcc_os_{index+1}.pdf', 'wb') as fp:
                    fp.write(ds.EncapsulatedDocument)
                images = convert_from_path(f'gcc/gcc_os_{index+1}.pdf')

                for i in range(len(images)):
                    # Save pages as images in the pdf
                    images[i].save(f'gcc/gcc_os_{index+1}.jpg', 'JPEG')

                im = Image.open(f'gcc/gcc_os_{index+1}.jpg')
                ...
                im.save(f"gcc/gcc_os_{index+1}.jpg")

                ocr = PaddleOCR(use_angle_cls=True, lang='en') # need to run only once to download and load model into memory
                img_path = f'gcc/gcc_os_{index+1}.jpg'
                ...
                print('-----------')
upload_blob('stanfordoptimagroup', 'gcc_text_extraction_os.csv', 'gcc_text_extraction_os.csv',verbose=False)
gcc_text_extraction = pd.read_csv('gcc_text_extraction_os.csv')
gcc_text_extraction = pyarrow_dtype_format_correction(gcc_text_extraction)
saved_table_id = 'imaging'+f".gcc_text_extraction_os"
